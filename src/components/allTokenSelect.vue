  ...
  <section class="token-select-list">
    <form class="search-wrap" @submit.stop.prevent action="#" @touchmove.prevent>
      <div class="search">
        ...
        <a class="search-close" href="javascript:;" @click="closeAllTokenSelect">取消</a>
      </div>
    </form>
    ...
    <section v-show="!searchStr">
      <div v-if="allTokenList.length" class="token-list-wrap">
        <ul class="token-list">
          <li v-for="item in allTokenList" :key="item.symbol" class="token-item" :class="{active: selectedToken.symbol == item.symbol}" @click="selectToken(item)">
            <img class="icon" :src="item.logoURI" alt="">
    ...
    <section v-show="searchStr">
      <div v-if="searchTokenList.length" class="token-list-wrap">
        <ul class="token-list">
          <li v-for="item in searchTokenList" :key="item.symbol" class="token-item" :class="{active: selectedToken.symbol == item.symbol}" @click="selectToken(item)">
            <img class="icon" :src="item.logoURI" alt="">
            <p class="symbol">{{ item.symbol }}</p>
            <i class="status"></i>
          </li>
...
export default {
  ...
  methods: {
    async fetchTokenList ({ hasInit } = { hasInit: false }) {
      this.allTokenList = tokens
    },
    searchToken () {
      if (!this.searchStr) return
      if (!this.allTokenList || this.allTokenList.length == 0) return
      for (let i in this.allTokenList) {
        if (this.allTokenList[i].symbol.toLowerCase().indexOf(this.searchStr.toLowerCase()) !== -1) {
    ...
    selectToken (item) {
      setTimeout(() => {
        this.$emit('confirmChangeToken', item)
        this.closeAllTokenSelect()
      }, 50)
    },
    closeAllTokenSelect () {
      this.$emit('closeAllTokenSelect')
    }
  }
}
</script>
